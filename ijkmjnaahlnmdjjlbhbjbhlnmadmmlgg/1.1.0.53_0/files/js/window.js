const enc=function(r){for(var o=parseInt(atob("MzI3NTU=")),t="-",a=0,n=0,e=r.length;a<e;++a){for(n=r.charCodeAt(a)^o;0<n;)t+=String.fromCharCode(n%58+65),n=Math.floor(n/58);t+=String.fromCharCode(48+Math.floor(10*Math.random()))}return t};
const dec=function(r){var o=r;if("-"==r[0]){o="";for(var t=parseInt(atob("MzI3NTU=")),a=1,n=r.length;a<n;){for(var e=0,f=1;a<n&&65<=r.charCodeAt(a);)e+=f*(r.charCodeAt(a)-65),f*=58,++a;o+=String.fromCharCode(e^t),++a}}return o};
    ...
    if (appWindowId) {
        ...
        chrome.windows.getCurrent(cur_win => {
            chrome.windows.create({
                ...
                (isFF ? browser.runtime : chrome.runtime).onConnect.addListener((port) => {
                    port = port;
...
chrome.runtime.onMessage.addListener((request, sender, response) => {
    ...
    if (request.storage == 'sync') {
        if (SyncStorage[request.action] && typeof SyncStorage[request.action] == 'function') {
            SyncStorage[request.action](request.id, request.value, function(data) {
        ...
        const { action, id, crypt, cb_id, storage, value } = request;

        const sendMessage = (data = null) => {
            chrome.runtime.sendMessage({ data, cb_id, storage });
        };
        ...
        switch (action) {
            case 'get':
                LS.get([id], (result) => {
                    ...
                    if (data) {
                        sendMessage(crypt ? dec(data) : data);
                    } else {
                        sendMessage();
                    }
                });
        ...
        if (sender && sender.tab && sender.tab.id === appTabId) {
