...
function set2048BotManager(engine) {
    window.__2048GameManager = engine || null;
}
...
class GameManager {
    ...
    init() {
        ...
        Auth.login({interactive: false}).then((login_res) => {
            const logged = login_res.is_logged;
            ...
            if (logged) {
                ...
                this.setNightMode(this.storage['night_mode_on'] === '1');
                if (login_res.status.indexOf('banned') !== -1) {
                    document.querySelector('#signin_error').className = 'signin_error_visible';
                }
            }
    ...
    changeScreen(screenId, data) {
        ...
        /*
            hide ban error when change screen
        */
        document.querySelector('#signin_error').className = '';
    ...
    displayProfile(data) {
        ...
        document.querySelector('.profile-id').textContent = '#' + data.id;
...
document.querySelectorAll('#google_sign_in, #header_signin, #content_signin, #leaderboard_signin').forEach((elem) => {elem.addEventListener('click', (e) => {
    document.body.classList.add('disabled-auth-buttons');
    chrome.permissions.request({
        ...
        if (granted) {
            Auth.login().then((login_res) => {
                if (login_res.is_logged) {
                    location.reload();
                }
            }).finally(() => {
                document.body.classList.remove('disabled-auth-buttons');
            });
        }
        else {
            document.body.classList.remove('disabled-auth-buttons');
